...
это нужно. Два пограничных исключения: голое целое за пределами
гарантированного диапазона i64 или голое десятичное, переполняющееся
до non-finite на binary64, вместо этого остаётся String — одной формы
недостаточно, чтобы стать числом, если значение не помещается.
Десятичное, которое вместо этого уходит в *underflow* (слишком близко
к нулю, чтобы быть представимым), всё равно становится Float,
округлённым до знакового `0.0`, а не String.
...
String — с двумя исключениями на числовых границах: целое за
пределами гарантированного диапазона i64 или десятичное,
переполняющееся до non-finite на binary64, вместо этого остаётся
String — без молчаливого переполнения и без исключения при разборе.
Десятичное, которое уходит в *underflow* (слишком близко к нулю,
чтобы быть представимым), всё равно становится Float, округлённым до
знакового `0.0` — оно не откатывается к String.
...
десятичные числа), а переполняющий литерал за пределами
поддерживаемого уходит в String.
...
[`versions/<v>/tests/`](versions/0.6/tests/), разбитым на до трёх
фикстурных категорий плюс один верхнеуровневый файл метаданных.
Runner соответствия MUST обходить каждую фикстурную категорию,
присутствующую в целевой версии — молчаливый пропуск незнакомой
категории даёт ложно-зелёный результат, что хуже,
чем полное отсутствие фикстур для неё.

- **`boundary-fixtures.json`** *(0.7+, не фикстурная категория)* —
  плоский список путей фикстур `valid/`, про которые известно, что они
  проверяют границу числового домена (спека § 5.2, § 8.1, § 8.2),
  например i64-переполняющий или Float-переполняющий литерал. Он
  лежит в корне `tests/`, а не внутри `valid/`, специально для того,
  чтобы runner, перечисляющий `valid/**/*.json` как фикстуры, никогда
  не принял его за таковую. Перечисление фикстуры там не говорит, каким
  должен быть вывод реализации с более широким доменом — только то,
  что такая реализация освобождена от побайтового совпадения с парой
  `.json`/`.canonical.ktav` этой фикстуры; реализация с минимальным
  доменом MUST по-прежнему совпадать с ней точно, как с любой другой
  фикстурой.
...
Реализация соответствует версии, если проходит каждый тест из каждой
категории, присутствующей в наборе этой версии. Подключайте
