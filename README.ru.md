...
конечные Float с конечным десятичным представлением, которое точно
проходит round-trip; минимальное binary64 использует `roundTiesToEven`.
...
допускаемый конечный Float MUST иметь конечный десятичный кандидат,
точно проходящий round-trip, а минимальное binary64 использует
`roundTiesToEven`. Конечное значение хост-представления без такого
кандидата, например точная рациональ `1/3`, не является Ktav Float.
