...
это нужно. Одно пограничное исключение: голое число за пределами
гарантированного диапазона (i64 для Integer, binary64 для Float)
тоже остаётся String — одной формы недостаточно, чтобы стать
числом.
...
String — если только число не выходит за гарантированный диапазон
(i64 для Integer, binary64 для Float): тогда оно тоже остаётся
String.
...
возвращается как `0.5`, а `1e2` — как `100`. Голое целое в пределах
...
десятичные числа), а литерал за пределами поддерживаемого уходит в
...
верхнеуровневых категорий. Runner соответствия MUST обходить каждую
категорию, присутствующую в целевой версии — молчаливый пропуск
незнакомой категории даёт ложно-зелёный результат, что хуже,
...
- **`valid/`** — парсящиеся документы. Каждый случай — тройка
  ...
  же `Value`. Порядок полей объекта значим. Несколько фикстур
  проверяют границу числового домена (например, i64-переполняющий
  литерал), на которой реализации с более широким доменом разрешено
  законно расходиться с парой `.json`/`.canonical.ktav`;
  `boundary-fixtures.json` в том же каталоге — машиночитаемый,
  замкнутый список того, какие именно это фикстуры и каков их
  альтернативный исход (спека § 8.1, § 8.2).
...
- **`unrepresentable/`** *(с 0.7)* — `Value`, которые
  ...
  JSON такого литерала нет), `unrepresentable_reason`
