...
version 0.5.0. Implementations in any programming language may claim
"Ktav 0.5.0 compliance" iff they satisfy every normative statement
below.
...
Compared with 0.1.x, version 0.5.0:
...
and needs no escape — there is no `\#` escape sequence in 0.5.0.
...
item that appears inside an inline compound, § 5.8), a backslash byte
`\` begins an **escape sequence**. The following **eight** escape
sequences are recognised; each is replaced by the indicated byte
before further classification (§ 5.2):
...
- Keys (the key portion before the first `:` of a pair, § 5.3).
...
<key-char>      ::= any UTF-8 code point except
                    ...
                    <key-char>+. Internal whitespace inside the segment
...
13. If the body matches the **integer literal** grammar (§ 3.6) and
    ...
    outside the i64 range; a 0.5.0-conformant parser running on a
...
- `key` is one or more dot-separated **segments** (`key-char+`).
  Each segment MUST be non-empty and MUST NOT contain a separator
  byte (`:`, `,`), a compound delimiter (`{`, `}`, `[`, `]`,
  `(`, `)`), or a line terminator. The `#` byte is allowed inside
  a segment but two consecutive `#` bytes at the trimmed start of
  a line form a comment marker (§ 3.4) and so an `##` substring
  at line start cannot be a key.
...
A key segment is emitted verbatim, after the trimming rule of § 4
has been applied at parse time. Internal whitespace is preserved.
Dotted keys are NOT re-expanded: a Value parsed from `a.b.c: 1` is
indistinguishable in the Value model from one parsed from
`a: { b: { c: 1 } }`, and the canonical writer chooses the
explicit nested form (not the dotted form). The `.` byte is the
key-segment separator and so does not appear inside a canonical
key segment.
...
error labelled `InlineNonEmptyCompound` when parsing 0.5.0 documents.
...
A `\X` form inside an inline scalar value (§ 3.7) where `X` is not
one of `\`, `,`, `}`, `]`, `{`, `[`, `n`, `r` is a
`BadEscapeSequence` error. End-of-line directly after a backslash
(i.e. `\` with no following byte before the line terminator) is
also a `BadEscapeSequence` error — the inline scalar context never
crosses a line boundary (§ 3.2).
...
An implementation may claim **Ktav 0.5.0 compliance** at one or more
...
- Accepts every fixture under `versions/0.5/tests/valid/` and
  produces a Value equivalent to the corresponding `name.json`
  oracle.
- Rejects every fixture under `versions/0.5/tests/invalid/` with
...
- For each fixture under `versions/0.5/tests/valid/`, produces —
...
version-pragma SHOULD be 0.5.0.
...
correctness footgun. The 0.5.0 escape set is the **minimal closed
set** required to write any of the structurally significant
characters of the inline form (`,`, `}`, `]`, `{`, `[`) and the
literal backslash (`\\`), plus two convenience escapes (`\n`, `\r`)
for embedded newlines. Other byte values are written literally.
...
Keys, multi-line scalars, and multi-line strings have no escape
processing at all — the lexical layout makes escape unnecessary in
those contexts.
