...
本文档规定格式 0.5.0 版本的语法与语义。任何编程语言的实现可声明
「Ktav 0.5.0 兼容性」,当且仅当满足以下每一条规范性声明。
...
内,反斜杠字节 `\` 开始一个 **escape 序列**。识别以下**八个** escape
序列;每个在进一步分类(§ 5.2)之前替换为指定字节:
...
- `<key-char>`:任意 UTF-8 码点,但不允许 ASCII 控制字节 < 0x20
  ...
  `(` `)` `:` `,`。ASCII 空格 (0x20) 与制表符 (0x09) **允许** 在键
  段内出现,因此键可包含内部空白(例如 `first name`)。
- **键段修剪**:键段在 `<key-char>+` 校验前被**修剪**掉前后的
...
13. 若体匹配**整数字面量**语法(§ 3.6)且数值至少落在 i64 范围内
    ...
    运行于严格 i64 后端的 0.5.0 解析器 MUST 将这类溢出体归入
    规则 15。
...
详见英文。键 MUST 由 `key-char+` 组成,每段不为空;不允许 `:`、
`,`、`{`、`}`、`[`、`]`、`(`、`)`、行终止符。`#` 单字符在段内
允许;`##` 仅在 trim 行首构成注释标记。
...
键段在 § 4 trim 已于解析时应用后 verbatim 输出。段内空白保留。
点分键**不**反展开:从 `a.b.c: 1` 解析得到的 Value 在 Value 模型
中与从 `a: { b: { c: 1 } }` 得到的不可区分,规范 writer 选择显式
嵌套形式。
...
有效(§ 5.8)。实现 MUST NOT 在解析 0.5.0 文档时输出标签为
...
MUST NOT 在解析 0.5.0 文档时输出标签为 `InvalidTypedScalar`
的错误。
...
`BadEscapeSequence`(§ 3.7)。反斜杠后紧随行末(即 `\` 与行终止符
之间无字节)亦为 `BadEscapeSequence` 错误 —— inline 标量上下文
不跨越行边界(§ 3.2)。
...
实现可在以下一个或多个层级声明 **Ktav 0.5.0 兼容性**。
...
- 接受 `versions/0.5/tests/valid/` 下每个 fixture 并产生与
  对应 `name.json` 等价的 Value。
- 拒绝 `versions/0.5/tests/invalid/` 下每个 fixture,错误类别
...
- 对 `versions/0.5/tests/valid/` 下每个 fixture,在给定从
