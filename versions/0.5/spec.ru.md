...
0.5.0. Реализации на любом языке программирования могут заявить
«соответствие Ktav 0.5.0» если и только если они выполняют каждое
...
По сравнению с 0.1.x, версия 0.5.0:
...
экранирования: escape-последовательности `\#` в 0.5.0 нет.
...
§ 5.8), байт обратной косой `\` начинает **escape-последовательность**.
Распознаются следующие **восемь** escape-последовательностей; каждая
заменяется указанным байтом перед дальнейшей классификацией (§ 5.2):
...
- Ключах (часть ключа перед первым `:` пары, § 5.3).
...
<key-char>      ::= любая UTF-8 кодовая точка, за исключением:
                    ...
                    <key-char>+. Внутренние пробельные байты внутри
...
13. Если тело соответствует грамматике **integer literal** (§ 3.6)
    ...
    на Integer-типизацию вне i64-диапазона; 0.5.0-парсер на
...
- `key` — один или более точко-разделённых **сегментов** (`key-char+`).
  Каждый сегмент MUST быть непустым и MUST NOT содержать
  разделительный байт (`:`, `,`), разделитель составных
  (`{`, `}`, `[`, `]`, `(`, `)`), или завершитель строки. Байт
  `#` внутри сегмента разрешён, но два последовательных байта `#`
  в начале обрезанной строки образуют маркер комментария (§ 3.4),
  поэтому подстрока `##` в начале строки не может быть ключом.
...
Сегмент ключа выводится verbatim, после применения trim (§ 4) на
парсинге. Внутренние пробелы сохраняются. Точечные ключи НЕ
...
меткой `InlineNonEmptyCompound` при парсинге 0.5.0-документов.
...
меткой `InvalidTypedScalar` при парсинге 0.5.0-документов.
...
`BadEscapeSequence` (§ 3.7). Конец строки сразу после обратного
слэша (т.е. `\` без следующего байта до завершителя строки) — также
ошибка `BadEscapeSequence`; inline-скалярный контекст не пересекает
границу строки (§ 3.2).
...
Реализация может заявлять **соответствие Ktav 0.5.0** на одном или
...
- Принимает каждую фикстуру из `versions/0.5/tests/valid/` и даёт
  Value, эквивалентное соответствующему `name.json` оракулу.
- Отвергает каждую фикстуру из `versions/0.5/tests/invalid/` с
...
- Для каждой фикстуры из `versions/0.5/tests/valid/` выдаёт —
...
version-pragma SHOULD быть 0.5.0.
