...
0.6.0. Реализации на любом языке программирования могут заявить
«соответствие Ktav 0.6.0» если и только если они выполняют каждое
нормативное утверждение ниже.

0.6.0: ключи теперь обрабатывают escape-последовательности (§ 3.7);
добавлены `\.` и `\:`; **ломающее** — литеральный обратный слэш в
ключе теперь требует `\\`.
...
По сравнению с 0.1.x, версия 0.6.0:
...
По сравнению с 0.5.0, версия 0.6.0:
...
ASCII пробел (`0x20`) и горизонтальная табуляция (`0x09`) являются
**пробельными** байтами. Реализации MAY дополнительно трактовать другие
Unicode-символы `White_Space` как пробельные; переносимые документы
SHOULD использовать только пробел и табуляцию.
...
экранирования: escape-последовательности `\#` в 0.6.0 нет.
...
следующие **десять** escape-последовательностей; каждая заменяется
указанным байтом перед дальнейшей классификацией (§ 5.2 для значений)
или разделением на сегменты (§ 4 для ключей):
...
Два новых escape для ключей (`\.`, `\:`) позволяют сегменту ключа
...
<key-char>      ::= любая UTF-8 кодовая точка, за исключением:
                    ...
                    от ведущих и хвостовых ASCII-пробельных байтов
                    (пробел 0x20, табуляция 0x09) перед валидацией по
                    <key-token>+. Внутренние пробельные байты внутри
                    сегмента сохраняются verbatim. Сегмент, пустой
                    после обрезки, — ошибка EmptyKey (§ 6.5).

                    Обработка escape в ключах: правило <key-escape>
                    обрабатывает те же десять escape-последовательностей,
                    что и § 3.7. Обратный слэш `\` является escape-лидом;
                    `\.` даёт литеральную точку (НЕ разделяет сегменты
                    пути); `\:` даёт литеральное двоеточие (НЕ действует
                    как разделитель пары); `\\` даёт литеральный
                    обратный слэш. Любая другая форма `\X` в ключе —
                    ошибка `BadEscapeSequence` (§ 6.13).
...
- `(ws)` — ноль или более ASCII-пробельных байтов.
- `1*ws` — **один или более** ASCII-пробельных байтов.
...
13. Если тело соответствует грамматике **integer literal** (§ 3.6)
    ...
    на Integer-типизацию вне i64-диапазона; 0.6.0-парсер на
...
- **Stripped (`( … )`)**: парсер вычисляет **минимальный ведущий
  ...
  префикс из каждой строки. Затем строки соединяются одиночными
  `\n`-байтами. Замыкающие пробелы в каждой строке сохраняются.
  Пустые строки внутри блока вносят пустые строки в результат
  объединения.
- **Verbatim (`(( … ))`)**: парсер соединяет содержательные строки
  байт-в-байт через одиночные `\n`-байты; обрезка пробелов не
  выполняется.
...
- **Float:** выбранная текстовая форма соответствует одной из
  ...
  Каноническая текстовая форма — **кратчайшее десятичное
  разложение, однозначно идентифицирующее Value**, вычисленное
  алгоритмом класса Ryu / Grisu / Steele-White. Для IEEE 754
  binary64 это кратчайшая десятичная, round-trip'ующая в тот же
  binary64.

  - Для значений с малой абсолютной экспонентой используется
    форма `digits "." digits` — например `1.5`, `42.0`, `0.5`.
  - Для значений с большой абсолютной экспонентой используется
    форма с экспонентой — например `1e9`, `2.5e10`, `1.5e-3`.
...
меткой `InlineNonEmptyCompound` при парсинге 0.6.0-документов.
...
меткой `InvalidTypedScalar` при парсинге 0.6.0-документов.
...
`{`, `[`, `n`, `r`, `.`, `:` — ошибка `BadEscapeSequence`. Конец
...
Реализация может заявлять **соответствие Ktav 0.6.0** на одном или
...
version-pragma SHOULD быть 0.6.0.
