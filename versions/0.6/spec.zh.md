...
本文档规定格式 0.6.0 版本的语法与语义。任何编程语言的实现可声明
「Ktav 0.6.0 兼容性」,当且仅当满足以下每一条规范性声明。

0.6.0:键现在处理 escape 序列(§ 3.7);新增 `\.` 与 `\:`;
**破坏性** —— 键中的字面反斜杠现在需要 `\\`。
...
13. 若体匹配**整数字面量**语法(§ 3.6)且数值至少落在 i64 范围内
    ...
    运行于严格 i64 后端的 0.6.0 解析器 MUST 将这类溢出体归入
    规则 15。
...
- **Float**:选取的文本形式匹配 § 3.6 的两个候选之一:
  ...
  规范文本形式是**最短的、能唯一确定 Value 的十进制展开**,由
  Ryu / Grisu / Steele-White 类算法计算。对 IEEE 754 binary64
  实现,即与同一 binary64 round-trip 的最短十进制。

  - 绝对指数较小的值,使用 `digits "." digits` 形式 —— 如 `1.5`、
    `42.0`、`0.5`。
  - 绝对指数较大的值,使用指数形式 —— 如 `1e9`、`2.5e10`、
    `1.5e-3`。
...
有效(§ 5.8)。实现 MUST NOT 在解析 0.6.0 文档时输出标签为
...
MUST NOT 在解析 0.6.0 文档时输出标签为 `InvalidTypedScalar`
的错误。
...
实现可在以下一个或多个层级声明 **Ktav 0.6.0 兼容性**。
