...
- **Float** — числовой скаляр, несущий числовое значение.
  ...
  (например, decimal произвольной точности). Внутреннее
...
Для `NonFiniteFloat` фикстуры нет: у JSON — формата, в котором
написан `.json` oracle фикстур, — нет портируемого литерала для NaN
или Infinity (реализации, принимающие голые токены `NaN` /
`Infinity` как расширение, расходятся в их round-trip'е), поэтому
этот случай задокументирован здесь только прозой, без
машинопроверяемой фикстуры.
...
- Для каждой фикстуры из `versions/0.7/tests/valid/` выдаёт —
  ...
  вывод, равный `name.canonical.ktav`. Это требование относится к
  Value, которым реализация действительно владеет, а оно —
  поскольку § 8.1 определяет эквивалентность фикстур на минимально
  требуемом числовом домене § 5 (Integer i64, Float binary64) —
  может законно отличаться от минимально-доменного Value,
  описанного `.json`-оракулом фикстуры, именно на тех
  boundary-probing фикстурах, что названы в § 8.1 (например, тело
  `i64_overflow_to_string`, классифицируемое минимальным доменом
  как String, а более широким — как Integer). Именно для такой
  фикстуры реализация с более широким доменом MAY выдать вывод,
  отличный от фиксированного `name.canonical.ktav` этой фикстуры —
  например, Integer записывается канонически голым, без
  raw-маркера (§ 5.9.5), — при условии, что этот вывод является
  корректной канонической формой (§ 5.9) для действительно
  удерживаемого Value и остаётся внутренне корректным и
  детерминированным для собственного домена реализации. Такая
  реализация остаётся writer-conforming, пока расхождение
  ограничено теми же boundary-probing фикстурами, что названы в
  § 8.1, и не является произвольным или недокументированным
  расхождением в остальном.
...
- **Изменено:** § 8.2 (вместе с § 5.9.5) — байт-точное требование
  ...
  домене, зеркалящей § 8.1: ровно на boundary-probing фикстурах,
  названных в § 8.1, Value, разобранный реализацией с более
