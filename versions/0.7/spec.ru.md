...
- **`key: <bytes>` (обычный разделитель + один пробел + тело
  ...
  точности `()` или `(())` (которые shortcut'ы § 5.7
  переинтерпретировали бы как пустую String, а не как этот
  буквальный двух- или четырёхсимвольный текст).
- **`key:: <bytes>` (raw-маркер):** когда байты — непустая
  ...
  с `{` или `[`), либо как пустая String через shortcut'ы § 5.7
  (тело в точности `()` или `(())`), либо любая иная не-String
  классификация.
...
- **Raw-маркерный элемент:** `:: <bytes>` — когда тело иначе было
  ...
  inline-составное, либо (через shortcut'ы § 5.7) как пустая
  String.
