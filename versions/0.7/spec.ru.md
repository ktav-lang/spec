...
- **Незакрытые квотированные сегменты.** Если символ кавычки
  ...
  2. **Позиция inline-ключа внутри `{...}` (§ 5.8.2) — включая первую
     ...
     или `MalformedInlineCompound` (§ 6.11 / § 6.12) — НИКОГДА не
     `UnterminatedQuotedKey` (§ 6.16 явно исключает этот контекст):
...
позиции сегмента ключа), — либо экранирование `##`-префикса
(пункт 4 ниже) либо экранирование краевого пробела (пункт 3 ниже);
...
корня-Array). Необходимость экранировать литеральный обратный слэш,
...
- Пробельная кодовая точка § 3.3 на первой или последней позиции
  ...
  требует.
- Если выводимый сегмент — первый сегмент ключа, и его
  ...
  не вместо этого. Структурный байт или краевая пробельная
  кодовая точка в другом месте `##`-ключа экранируется точно так же, как и в
...
скобкам. Проверка баланса содержимого позади `UnterminatedInlineCompound`
(§ 6.11) / `MalformedInlineCompound` (§ 6.12) уже непрозрачна для
кавычек именно по этой причине (пункт «Inline-пары» § 5.3.3) и
сообщает одну из этих двух категорий вместо этого — никогда не
`UnterminatedQuotedKey` — поскольку дефект на уровне составного
значения — вот что читатель реально может увидеть и исправить
(добавить недостающий закрывающий символ или недостающий закрывающий
символ кавычки, на что бы ни указывал диагноз самого составного
значения); нет отдельного дефекта «ключ внутри тоже был незакрыт»,
...
(или опасность `##`-префикса или краевого пробела), а не оставляет
голую-форму-с-экранированием равноценным каноническим выбором:
требование детерминизма (§ 5.9) означает, что у писателя нет свободы
...
- **Добавлено:** Квотированные ключи (§ 5.3.3) — сегмент ключа MAY
  ...
  круглой скобки, или экранирования `##`-префикса (например, `a\.b: 1`
