...
и поддерживающие один и тот же числовой домен (§ 5, § 8.1), MUST
давать одинаковый Value kind для одного и того же тела. Там, где их
числовые домены различаются, тело может пересечь границу домена
одной реализации, но не другой — выходящий за диапазон Integer- или
неконечный Float-литерал проваливается в String по правилам 13–14
для более узкого домена, но остаётся Integer или Float для более
широкого. Это не
исключение из детерминизма:
[`versions/0.7/tests/valid/boundary-fixtures.json`](tests/valid/boundary-fixtures.json)
называет полный, замкнутый набор фикстур, для которых допускается
такое зависящее от домена расхождение (§ 8.1, § 8.2); вне этого
названного набора гарантия одинакового kind действует безусловно.
...
обычного JSON-числа. Эта сентинел-форма зарезервирована ровно для
этой ситуации — никакой другой код причины и никакая фикстура под
`valid/` в ней никогда не нуждаются, поскольку каждый прочий Value,
определяемый § 5, имеет прямое отображение в JSON.
...
- Принимает каждую фикстуру из `versions/0.7/tests/valid/` и даёт
  ...
  [`versions/0.7/tests/valid/boundary-fixtures.json`](tests/valid/boundary-fixtures.json)
  — полный, машиночитаемый набор исключений: ровно для путей
  фикстур, перечисленных его ключами, реализация, поддерживающая
  более широкий домен, MAY вместо выдачи минимально-доменного
  `name.json` оракула выдать `wider_domain_kind` /
  `wider_domain_value`, названные манифест-записью этой фикстуры —
  например, i64-переполняющий литерал, классифицируемый минимальным
  доменом как String (`i64_overflow_to_string.json`), а более
  широким — как Integer, согласно манифест-записи этой фикстуры.
  Никакая фикстура вне этого манифеста и никакой альтернативный
  Value, кроме названного манифестом, не допускаются. Такая
  реализация остаётся parser-conforming, пока каждое расхождение —
  в точности названный манифестом исход для перечисленной в
  манифесте фикстуры, а не произвольное или недокументированное
  расхождение в остальном.
...
- Для каждой фикстуры из `versions/0.7/tests/valid/` выдаёт —
  ...
  описанного `.json`-оракулом фикстуры, ровно для фикстур,
  перечисленных в
  [`versions/0.7/tests/valid/boundary-fixtures.json`](tests/valid/boundary-fixtures.json)
  (например, тело `i64_overflow_to_string`, классифицируемое
  минимальным доменом как String, а более широким — как Integer,
  согласно манифест-записи этой фикстуры). Ровно для такой
  перечисленной фикстуры вывод реализации с более широким доменом
  MUST вместо этого быть байт-точным равным полю
  `wider_domain_canonical` этой фикстуры — например, значение
  Integer записывается канонически голым, без raw-маркера
  (§ 5.9.5), — никакой третий исход, свободно выбранный
  реализацией, не конформен. Такая реализация остаётся
  writer-conforming, пока каждое расхождение — в точности названный
  манифестом `wider_domain_canonical` для перечисленной в манифесте
  фикстуры, а не произвольное или недокументированное расхождение
  в остальном.
