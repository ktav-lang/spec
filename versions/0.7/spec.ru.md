...
- **Голый скалярный элемент:** `<bytes>` на своей строке на
  ...
  (§ 3.4), молча отбрасывая всю строку вместо ошибки.
...
представима в канонической multi-line форме; реализации MAY
выдавать любую форму, принимая, что свойство round-trip строго не
выполняется для таких патологических значений. Переносимые
...
его. Реализации MAY выдавать stripped-форму, принимая, что
свойство round-trip строго не выполняется для таких патологических
значений, точно так же, как и для случая, требующего обеих форм
выше. Переносимые документы SHOULD NOT полагаться на хвостовой
...
Реализации MAY выдавать stripped-форму, принимая, что свойство
round-trip строго не выполняется для таких патологических значений,
точно так же, как и для других случаев выше. Переносимые документы
...
- Если сырой текст ключа (первый сегмент, как он записан в
  ...
  writer MUST экранировать хотя бы первый `#` как `\u0023`:
  без экранирования правило 2 из § 5.1 прочитает строку как
  комментарий (§ 3.4) и молча отбросит её — этот байт не
  входит ни в список исключений `<key-char>`, ни находится на
  обрезаемой границе, но всё равно нуждается в экранировании,
  поскольку эта опасность действует на уровне диспетчеризации
  строк, выше грамматики сегмента ключа § 4 целиком.
