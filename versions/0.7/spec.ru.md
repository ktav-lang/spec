...
- Если сырой текст ключа (первый сегмент, как он записан в
  ...
  writer MUST экранировать ровно первый `#` как `\u0023` и
  оставить любой другой байт сегмента — включая второй `#` —
  неэкранированным: без экранирования правило 2 из § 5.1
  прочитает строку как комментарий (§ 3.4) и молча отбросит
  её — этот байт не входит ни в список исключений
  `<key-char>`, ни находится на обрезаемой границе, но всё
  равно нуждается в экранировании, поскольку эта опасность
  действует на уровне диспетчеризации строк, выше грамматики
  сегмента ключа § 4 целиком. Экранирование ровно этого
  одного байта необходимо и достаточно: две
  writer-conforming реализации MUST производить идентичный
  префикс `\u0023#…`, никогда не `\u0023\u0023…` или другой вариант.
