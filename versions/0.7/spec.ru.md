...
- Принимает каждую фикстуру из `versions/0.7/tests/valid/` и даёт
  ...
  перечисляет пути фикстур, известные как boundary-probing —
  зондирующие границу числового домена (§ 5.2), — например
  `numbers/integer/i64_overflow_to_string`, чьё тело минимальный
  домен классифицирует как String, но более широкий домен MAY
  классифицировать как Integer. Реализация, поддерживающая только
  минимальный домен, MUST совпасть с каждой из этих фикстур в
  точности, как и с любой другой фикстурой `valid/`. Реализация,
  поддерживающая более широкий домен, освобождена от байт-в-байт
  совпадения с `name.json` перечисленной фикстуры — этот корпус не
  фиксирует, каким должен быть её Value там; § 5.2 уже формулирует
  общее правило (тот же домен ⇒ тот же kind, различающийся домен ⇒
  MAY различаться на пересечённой границе), а этот Value — то, что
  производит собственное корректное применение правил 13–14 § 5
  реализацией к телу фикстуры. Фикстура, не перечисленная в
  `boundary-fixtures.json`, не несёт такого освобождения ни для
  какой реализации, какого бы то ни было домена.
...
- Для каждой фикстуры из `versions/0.7/tests/valid/` выдаёт —
  ...
  вывод, равный `name.canonical.ktav`, кроме случая, когда фикстура
  перечислена в
  [`versions/0.7/tests/boundary-fixtures.json`](tests/boundary-fixtures.json),
  а реализация поддерживает числовой домен более широкий, чем
  минимально требуемый § 5, — согласно § 8.1 такая реализация может
  владеть иным Value для тела этой фикстуры, чем описывает
  минимально-доменный `.json` оракул (например, тело
  `i64_overflow_to_string`, удерживаемое как Integer, а не String).
  Для такой фикстуры этот корпус также не фиксирует точную
  требуемую байтовую последовательность: вывод реализации MUST быть
  правильной канонической формой (§ 5.9) того Value, которым она
  действительно владеет, — например, значение Integer записывается
  канонически голым, без raw-маркера (§ 5.9.5), — и оставаться
  внутренне согласованным и детерминированным для её собственного
  домена, но какие именно это байты для домена, отличного от
  минимального, этот общий, не зависящий от языка корпус не
  проверяет. Реализация, поддерживающая только минимальный домен,
  MUST совпасть с `.canonical.ktav` каждой записи
  `boundary-fixtures.json` в точности, как и с любой другой
  фикстурой `valid/`.
