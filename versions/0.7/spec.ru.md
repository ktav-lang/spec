...
6. Иначе, если первая содержательная строка после trim — **pair
   line** по § 5.3 (`key: …` / `key:: …`, включая точечные ключи)
   → корень — **Object** с этой строкой как первой парой.
...
- `key` — один или более **сегментов**, разделённых
  ...
  сегмента разрешён, но два последовательных байта `#` в начале
  обрезанной строки образуют маркер комментария (§ 3.4), поэтому
  подстрока `##` в начале строки не может быть ключом. Разделитель
  пары — первое **неэкранированное** `:` (или `::`) при
  сканировании слева направо.
...
Сегмент ключа, который пуст, содержит запрещённый символ или
коллидирует с маркером комментария в начале обрезанной строки,
порождает ошибку `InvalidKey` (§ 6.4) или `EmptyKey` (§ 6.5).
