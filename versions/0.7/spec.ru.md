...
намеренно не имеют **выделенной именованной**
escape-последовательности (нет `\t`) — ни одну букву не стоит
резервировать для байта, который и так допустим как литерал.
Табуляция — допустимый литеральный байт в ключах и скалярах (§ 4);
управляющие байты — это содержательные данные. String, содержащая
такие байты, представима через verbatim-многострочную форму
