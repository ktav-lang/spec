...
Маркер порядка байтов (U+FEFF) в самом начале документа, перед любым
другим байтом, MAY быть пропущен реализацией (в духе трактовки
RFC 8259 § 8.1 для JSON). Кодовая точка U+FEFF в любом другом месте
документа — обычное содержимое: § 3.3 не относит её к пробельным
символам.
...
содержимое String только через сам escape `\r`.
