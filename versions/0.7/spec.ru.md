...
(representable) Value — § 5.9.7 определяет узкое множество значений
String, которые не являются представимыми. Каноническая форма
является байт-детерминированной: для любого данного представимого
Value каждая реализация-эмиттер MUST производить одну и ту же
байтовую последовательность. Реализация-эмиттер MUST отклонять
непредставимое Value с ошибкой, а не сериализовать его: разрешить
эмиттеру произвольную/lossy кодировку для одного и того же
непредставимого Value значило бы нарушить только что заявленную
гарантию байт-детерминизма.
...
Value V (§ 5.9.7), производимого parser-conforming реализацией, при
