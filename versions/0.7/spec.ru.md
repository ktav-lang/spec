...
многострочная строковая форма `(…)` теперь обрезает завершающие
пробельные символы построчно — так же, как `(…)` уже поступала с
ведущими пробельными символами (§ 5.6) — **ломающее** для документов,
у которых значимое содержимое примыкало к ASCII-пробельному символу
на границе значения/ключа или на завершающей границе строки внутри
`(…)`, хотя на практике не ломающее ни для одного релиза 0.6.x
Rust-ядра, которое уже обрезало полный набор.
...
По сравнению с 0.1.x, версия 0.6.4:
...
По сравнению с 0.5.0, версия 0.6.4:
...
**Комментарий** — это строка, первые непробельные байты которой —
`##` (два ASCII `#`-байта). Остаток строки до завершителя строки
...
экранирования: escape-последовательности `\#` в 0.6.4 нет.

### 3.5 Пустые строки

**Пустая строка** — строка, состоящая только из пробельных байтов.
...
не часть escape (`A` + `1` декодируется в кодовую точку `U+0041`, за
которой следует литеральная цифра `1`, а не в пятизначный escape).
...
низкий суррогат в форме `\uXXXX`, либо низкий суррогат, которому не
предшествует высокий — это **одинокий суррогат**, и это ошибка
...
(см. ниже «Escape-последовательности НЕ обрабатываются в»).
...
именованную форму (`\.` вместо `.`, для согласованности с уже
существующими десятью) и использовать `\uXXXX` только для кодовых
точек без именованного escape.
...
<key-char>      ::= любая UTF-8 кодовая точка, за исключением:
                    ASCII управляющих байтов < 0x20 (кроме табуляции 0x09),
                    DEL (0x7F),
                    ...
                    (примечание: ASCII пробел 0x20 И табуляция 0x09
                    РАЗРЕШЕНЫ внутри сегмента ключа, так что ключ MAY
                    ...
                    пробельные байты внутри сегмента сохраняются
...
- `<sep-end>` — «как минимум один пробельный байт или конец строки».
...
- **Integer** — числовой скаляр, несущий целочисленное значение.
  ...
  произвольную точность). Каноническая текстовая форма Integer —
...
2. Если первая содержательная строка после trim — **замкнутый
   ...
   соответствующей `}` как последним непробельным байтом строки
   после trim) → корень **И ЕСТЬ** этот inline Object. Документ
...
3. Если первая содержательная строка после trim — **замкнутый
   ...
   соответствующей `]` как последним непробельным байтом строки
   после trim) → корень **И ЕСТЬ** этот inline Array. Документ
   MUST не иметь дальнейших содержательных строк; любая
   последующая непустая некомментарная строка — ошибка
   `OrphanLineAfterTopLevelInline` (§ 6.14).
...
13. Если тело соответствует грамматике **integer literal** (§ 3.6)
    ...
    на Integer-типизацию вне i64-диапазона; 0.6.4-парсер на
    строго-i64 бэкенде MUST помещать такие переполняющие тела в
    правило 15.
...
Две реализации-парсера, конформные спецификации, MUST давать
одинаковый Value kind для одного и того же тела.
...
- Сырой маркер `::` интерпретирует тело как литеральную String —
  ...
  (§ 5.8).
- `<sep-end>` требует как минимум один пробельный байт или конец
  строки после разделителя. Запись `key:value` / `key::value` (без
...
`DuplicateKey` (§ 6.2). Span ошибки MUST указывать на дублирующий
ключ, не на первое вхождение.

### 5.6 Многострочные строки

Многострочная строка открывается значением-начало `(` (stripped) или
`((` (verbatim) на строке, не содержащей других непробельных байтов
после опенера. Закрывающая строка — строка, обрезанная содержимое
которой в точности `)` (для stripped) или `))` (для verbatim).

- **Stripped (`( … )`)**: парсер вычисляет **минимальный ведущий
  пробел** среди непустых содержательных строк и удаляет этот
  префикс из каждой строки, затем удаляет замыкающий пробел (§ 3.3)
  из каждой строки. Затем строки соединяются одиночными `\n`-байтами.
  Пустые строки внутри блока вносят пустые строки в результат
  объединения. Пустая строка, состоящая только из пробельных байтов
  (по § 3.5), НЕ участвует в вычислении общего отступа; она вносит
...
Байт `{` или `[`, который **НЕ** является первым непробельным
байтом inline-значения (то есть находится в середине скаляра), —
литеральный символ и НЕ открывает вложенное составное. Решение
принимается один раз, когда парсер начинает читать inline-значение:
если первый непробельный байт `{` или `[`, значение — вложенное
...
*каноническую* Ktav-сериализацию любого Value. Каноническая форма
является байт-детерминированной: для любого данного Value каждая
реализация-эмиттер MUST производить одну и ту же байтовую
последовательность.
...
- **`key: <bytes>` (обычный разделитель + один пробел + тело
  ...
  которой не содержит `LF`, `CR`, ведущих/хвостовых пробелов,
  управляющих байтов (0x00–0x1F кроме 0x09 `TAB`), не является
  токеном, который § 5.2 классифицировала бы иначе чем String
  (число / `null` / `true` / `false`), и не начинается с `{`
  или `[` (что заставило бы § 5.2 диспетчеризовать тело как
  inline-составное).
- **`key:: <bytes>` (raw-маркер):** когда тело — непустая
  однострочная String, которая иначе была бы переинтерпретирована
  по § 5.2 — как число / `null` / `true` / `false`, как
  inline-составное (тело, начинающееся с `{` или `[`), или
  любая иная не-String классификация.
- **`key: <multi-line>`:** когда значение — String с `LF`, `CR`,
  ведущим/хвостовым пробелом или непечатным управляющим байтом.
- **`key: <compound>`:** когда значение — непустой Object / Array
  (§ 5.9.4) или пустые `{}` / `[]`.
...
- **Содержит `LF`, ведущий/хвостовой пробел, или управляющий байт
  (`0x00`–`0x1F` кроме `0x09` `TAB` и `0x0A` `LF`):** выводить как
  verbatim multi-line `((` … `))`. Опенер `((` выводится на строке
...
- **Содержит байт `CR` (`0x0D`):** Value **не представимо** в
  ...
  Реализации MAY выдать любое детерминированное кодирование для
  таких Value (или отвергнуть их), и round-trip свойство § 8.3
  не выполняется. Переносимые документы SHOULD NOT полагаться на
  байты `CR` в String-значениях.

Канонический эмиттер предпочитает verbatim multi-line `((` … `))`
для строк, требующих многострочного представления. Если какой-то
сегмент тела (после разбивки по `LF`) в точности `))`, verbatim
форма невозможна (этот сегмент закрыл бы блок); тогда канонический
эмиттер MUST переключиться на stripped форму `(` … `)` без
ведущего отступа (writer выводит сегменты тела на отступе 0, так
что вычисление общего отступа даёт ноль). Закрывающая `)` строка
выводится на внешнем отступе.

(Обоснование: stripped-форма с закрывателем `)` оставляет `))`
доступным как содержание, что — единственный способ представить
такую последовательность байтов в multi-line значении.)

String, тело которой требует обеих форм — содержит и сегмент
`))`, и сегмент `)` — не представима в канонической multi-line
форме; реализации MAY выдавать любую форму, принимая, что
свойство round-trip строго не выполняется для таких
...
- **Float:** выбранная текстовая форма соответствует одной из
  ...
  - если `abs < 1e-2` или `abs >= 1e7`, используется форма с экспонентой;
...
При выводе сегмента ключа writer MUST ре-экранировать любой байт,
который иначе был бы структурным или неоднозначным:

- `\` → `\\` (обратный слэш — escape-лид)
- `.` → `\.` (точка — разделитель пути)
- `:` → `\:` (двоеточие — разделитель пары)
...
Сегмент ключа, содержащий литеральный `.`, `:` или `\`, выводится
в экранированной форме, чтобы повторный парсинг дал тот же ключ.
...
меткой `InlineNonEmptyCompound` при парсинге 0.6.4-документов.
...
меткой `InvalidTypedScalar` при парсинге 0.6.4-документов.

### 6.10 Отсутствие пробела после разделителя

В многострочной pair line разделитель `:` / `::` MUST сопровождаться
как минимум одним пробельным байтом или концом строки. Inline-пары
...
суррогат, которому не предшествует высокий.
...
Реализация может заявлять **соответствие Ktav 0.6.4** на одном или
...
- Принимает каждую фикстуру из `versions/0.6/tests/valid/` и даёт
  Value, эквивалентное соответствующему `name.json` оракулу.
- Отвергает каждую фикстуру из `versions/0.6/tests/invalid/` с
...
- Для каждой фикстуры из `versions/0.6/tests/valid/` выдаёт —
...
Следующее тождество MUST выполняться для каждого Value V,
производимого parser-conforming реализацией, при выводе и
повторном парсинге writer- и parser-conforming реализациями
того же домена Value:
...
фиксированная точка цикла parse-emit.
...
version-pragma SHOULD быть 0.6.4.
...
- **Изменено (нормативное уточнение, не ломающее ни для одного
  выпущенного релиза 0.6.x Rust-ядра):** § 3.3 меняется с
  ...
  обе стороны.
- **Изменено:** обрезка сегмента ключа в § 4 расширяется с ASCII-only
  ...
  пробелы) и § 4 (требовавшим ASCII-only именно для ключей).
...
- **Breaking:** Сегменты ключей обрезаются от ведущих/хвостовых
  ASCII-пробелов (§ 4).
- **Breaking:** Одиночный байт `CR` (без следующего `LF`) — это
  данные, не завершитель строки (§ 3.2).
...
1. **Многострочные строки `(…)` больше не сохраняют замыкающие
   ...
   на то, что замыкающие пробелы или табуляции внутри блока `(…)`
   сохраняются verbatim, переключите этот блок на `((…))`, которая
