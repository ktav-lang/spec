...
stripped 形式的关闭符碰撞)—— 不能以规范多行形式表示;实现 MAY
输出任一形式,接受 round-trip 性质对此病态值不严格成立。可移植
文档 SHOULD NOT 依赖此类内容。

自 0.7.0 起,若体含有修剪后恰为 `))` 的段(迫使采用上述 stripped
回退),且**同时**在任意内容行上还有尾部空白(§ 3.3),同样不可
表示:stripped 形式现在会在输出时去除该尾部空白,导致回退悄悄
丢失它。实现 MAY 输出 stripped 形式,接受 round-trip 性质对此
类病态值不严格成立,与上述两种形式皆需的情形处理方式相同。可
移植文档 SHOULD NOT 在同时需要修剪后恰为 `))` 的段的多行
String 内容中依赖尾部空白。
...
记录下来,与该形式已有的其他不可表示情形并列。实现 MAY 输出
stripped 形式,接受 round-trip 性质对此类病态值不严格成立,与
上述其他情形处理方式相同。可移植文档 SHOULD NOT 在同时需要修剪
...
- 若键的原始文本(第一个段,书写于行中的形式)以两字节序列
  `##` 开头,writer MUST 至少将第一个 `#` escape 为 `\u0023`:
  若不 escape,§ 5.1 规则 2 会将该行读作注释(§ 3.4)并悄悄
  丢弃 —— 该字节既不在 `<key-char>` 的排除列表中,也不位于
  被修剪的边界,但仍需要 escape,因为这一风险发生在逐行分发
  层面,高于 § 4 的整个键段语法。
