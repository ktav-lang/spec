...
- 若键的原始文本(第一个段,书写于行中的形式)以两字节序列
  `##` 开头,writer MUST 恰好将第一个 `#` escape 为 `\u0023`,
  并保持该段其余每个字节(包括第二个 `#`)不 escape:若不
  escape,§ 5.1 规则 2 会将该行读作注释(§ 3.4)并悄悄丢弃 ——
  该字节既不在 `<key-char>` 的排除列表中,也不位于被修剪的
  边界,但仍需要 escape,因为这一风险发生在逐行分发层面,高于
  § 4 的整个键段语法。恰好 escape 这一个字节既是必要的也是
  充分的:两个 writer-conforming 实现 MUST 产生相同的前缀
  `\u0023#……`,绝不能是 `\u0023\u0023……` 或其他变体。
