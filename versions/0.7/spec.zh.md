...
- `key` 由一个或多个以未 escape 的点分隔的**段**组成
  ...
  **escape 感知**的(§ 3.7、§ 4):`\` 是 escape 前导;`\.` 产生
  ...
  正是因为裸 `##` 会使该行在重读时无法按原意解析。pair 分隔符是
...
- **未终止的 quoted 段。** 若引号字符开启了一个段,而在行末之前
  ...
  2. **`{...}` 内的 inline 键位置(§ 5.8.2)—— 包括本身以 `{`
     ...
     `UnterminatedInlineCompound` 或 `MalformedInlineCompound`
     (§ 6.11 / § 6.12)—— 绝不是 `UnterminatedQuotedKey`(§ 6.16
     明确排除此上下文):复合值层面的缺陷才是读者实际能看到并
...
多行字符串)—— 或 `##` 前缀的 escape(下面第 4 条)或边缘空白
的 escape(下面第 3 条);或 (b) 该段解码后的内容以 `"`、`'`
...
类似情形)。需要 escape 字面反斜杠、LF、CR、控制字节或 DEL ——
...
本身永远无法完成括号平衡。`UnterminatedInlineCompound`(§ 6.11)/
`MalformedInlineCompound`(§ 6.12)背后的平衡内容检查正是出于这个
原因而已经对引号不透明(§ 5.3.3「inline pair」一条),因而报告的
是这两个类别之一 —— 绝不是 `UnterminatedQuotedKey` —— 因为复合值
层面的缺陷才是读者实际能看到并修复的(添加缺失的闭合符或缺失的
引号闭合符,具体以复合值自身的诊断为准);没有必要在其之上再命名
...
规范 writer(§ 5.9.10)一旦另需 STRUCTURAL escape(或
`##` 前缀、边缘空白隐患),就会优先选用 quoted 形式,而不是把
...
- **新增:** 带引号的键(§ 5.3.3)—— 键段 MAY 写成 `"…"`、`'…'`
  ...
  (`.` `:` `,` `{` `}` `[` `]`)、`(` / `)`、`##` 前缀或边缘空白
  做 escape,就会优先选用 quoted 形式(分隔符 `"`)而非
  bare-with-escape(仅 escape 反斜杠、LF、CR、控制字节或 DEL
  不会切换形式,因为加引号并不能去掉那个 escape)—— 这改变了此前
  需要 `\.` / `\:` / 括号 / 逗号 / 圆括号 escape,或 `##` 前缀
  escape 的每一个键的规范字节(例如 `a\.b: 1` 现在规范化为
  `"a.b": 1`,而非 `a\.b: 1`);既有的
