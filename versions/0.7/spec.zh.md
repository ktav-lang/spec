...
`(…)` 形式现在会逐行去除尾部空白,这与 `(…)` 此前已对每行前导空白
所做的处理一致(§ 5.6)——对于在值/键边界或 `(…)` 某行尾部边界处内容
恰好紧邻 ASCII 空白字符的文档而言,这是**破坏性**变更,但相对于每一个
0.6.x Rust 核心版本而言,实际上并非破坏性 —— 它们本就已经去除了完整
的集合。
...
字节属于普通内容,不属于该 escape(`A` 加 `1` 解码为码点 `U+0041`
后跟字面数字 `1`,而非五位 escape)。
...
- **变更(规范性澄清,相对每一个已发布的 0.6.x Rust 核心版本均非
  ...
  时在两个方向上均与此列表存在分歧。
- **变更:** § 4 的键段修剪从仅 ASCII 扩展到 § 3.3 的同一固定 25
  码点集合,解决了 § 3.3(此前已允许 Unicode 空白)与 § 4(此前
  专门要求键使用仅 ASCII)之间原本存在的矛盾。Rust 参考实现的实际
  修剪行为并未改变 —— 自 0.6.0 起它就已经修剪完整集合;改变的只是
  规范文本追上了代码。
