export default {
  en: `- \`(ws)\` stands for zero or more line-bounded \`ws\` code points defined
...
`,
  ru: `- \`(ws)\` — ноль или более определённых выше ограниченных строкой кодовых
...
`,
  zh: `- \`(ws)\` 表示零个或多个上文定义的行边界内 \`ws\` 码点;LF 和 CR
  不包括在内。
