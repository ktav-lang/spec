...
>>>>> lang=ru
   ...
   выбирает корень Object. Поскольку фаза 1 повторно использует
   правило сканирования разделителей § 4 дословно, а не отдельную
   реализацию, она бесплатно наследует учёт кавычек этого правила —
   содержимое quoted-сегмента остаётся непрозрачным для сканирования
   даже на этой НЕРЕШЁННОЙ первой строке, без отдельной логики,
   которую нужно было бы синхронизировать. Фаза 2 — единообразная
   валидация:
...
>>>>> lang=zh
   ...
   分隔符前的前缀在判定时不必是语法上合法的 `<key>`(§ 4),因此首行
   `a,b: 1` 仍选出 Object 根。因为阶段 1 逐字复用 § 4 的分隔符
   扫描规则,而非单独实现,它免费继承了该规则的引号感知 ——
   quoted 段的内容即使在这行尚未判定的首行上也对扫描保持
   不透明,无需再单独维护一套逻辑与之保持同步。阶段 2 是统一
   校验:Object 上下文
...
7. 否则,若首条内容行经 trim 后被识别为 **array-item line**
   (§ 5.4,除规则 4 / 5 已处理者外)—— 裸标量、原始标记项
   `:: …`、多行字符串开启符 `(` / `((`、空复合简写 `{}` / `[]` /
   `()` / `(())`,或未匹配规则 2 / 3 的闭合 inline 复合 —— →
   根为 **Array**,此行为首项。
