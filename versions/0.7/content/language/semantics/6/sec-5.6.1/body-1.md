...
любым эмиттером: чтобы воспроизвести такое содержимое, требуется
переключить многострочную строку на противоположную форму, поскольку
парсер никак не может отличить содержательную строку из одиночной `)`
от закрытия stripped-формы.

>>>>> lang=zh

stripped 块内,若某内容行 trim 后恰为 `)`,该行将与关闭符无法区分;
写入端 MUST 在需要写出这样的内容行时切换到 verbatim 形式。类似的
规则适用于 verbatim 块内 trim 后恰为 `))` 的内容行。

解析器行为对称:若 stripped 块内某内容行 trim 后恰为 `)`,解析器
MUST 在该行关闭块;若 verbatim 块内某内容行 trim 后恰为 `))`,
亦同。因此任何输出实现都 MUST 遵守上述写入端规则 —— 若要表达
此类内容,多行字符串必须切换为另一种形式,因为解析器无法区分
仅含 `)` 的内容行与 stripped 形式的关闭符。
