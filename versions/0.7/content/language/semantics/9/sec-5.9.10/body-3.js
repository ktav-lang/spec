export default {
  en: `When bare form is selected, the writer MUST re-escape every code
...
`,
  ru: `Если выбрана голая форма, writer MUST ре-экранировать каждую
...
`,
  zh: `- 有命名 escape(§ 3.7)的字节使用该 escape:\`\\\` → \`\\\\\`、
  \`.\` → \`\\.\`、\`:\` → \`\\:\`、\`,\` → \`\\,\`、\`{\` → \`\\{\`、\`}\` → \`\\}\`、
  \`[\` → \`\\[\`、\`]\` → \`\\]\`、LF → \`\\n\`、CR → \`\\r\`。
