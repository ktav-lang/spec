...
- Две или более последовательных запятых (`{a: 1,, b: 2}`,
  `[1,, 2]`).
- Пустой inline-элемент массива, то есть запятая, перед которой
  нет значения (`[a,, b]`); исключение для замыкающей запятой
  § 5.8 по-прежнему применимо к одиночной запятой непосредственно
  перед закрывающим разделителем.
- Прочие структурные дефекты, не дающие `UnterminatedInlineCompound`
  ...
  `{a 1, b: 2}`).
- Непробельное содержимое после соответствующего закрывающего символа
...
- 空 inline 数组项,即前面没有值的逗号(`[a,, b]`);§ 5.8 关于
  闭合符前单个尾部逗号的例外仍适用。
