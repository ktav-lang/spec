...
корень — Object. Когда конкретная причина, по которой разделитель
не найден, — незакрытый квотированный сегмент ключа (§ 5.3.3),
вместо этого сообщается более специфичная \`UnterminatedQuotedKey\`
(§ 6.16).

`,
  zh: `
\`MissingSeparator\` —— 派发到 pair-line 模式但无**未 escape** 的 \`:\` 分隔符的行。
适用于打开的多行 Object 体内,或顶层根为 Object 时。当未找到分隔符
的具体原因是未终止的 quoted 键段(§ 5.3.3)时,改为报告更具体的
