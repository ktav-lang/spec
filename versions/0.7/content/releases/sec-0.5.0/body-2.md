...
- **Breaking:** Удалены типизированные маркеры `:i` и `:f`.
  ...
  (§ 3.6, § 5.2). Сырой маркер `::` сохранён для принудительной
  String.
- **Breaking:** Комментарии теперь используют `##` (два ASCII-байта
  `#`) и MUST занимать отдельную строку (§ 3.4). Одиночный байт `#`
  не имеет специального значения. Замыкающие комментарии после
  содержательной строки не поддерживаются.
- **Breaking:** Одиночный `{` / `[` на первой содержательной
  ...
  Array элемент внутри корневого Array; JSONL-стиль (несколько
  top-level объектов) больше не принимается.
- **Breaking:** Float Values больше не сохраняют текстовую форму;
  применяется числовая канонизация (§ 3.6, § 5.2, § 5.9.8). Модель
  Value несёт числовое значение; канонический writer выводит
  детерминированную текстовую форму.
- **Breaking:** Сегменты ключей обрезаются от ведущих/хвостовых
  ASCII-пробелов (§ 4). Сегмент, пустой после обрезки, — это
  `EmptyKey`.
- **Добавлено:** Inline-составные — `{key: value, key2: value}` и
  `[v1, v2, v3]`, с необязательной замыкающей запятой (§ 5.8).
  Inline-форма пригодна как значение, как элемент массива или как
  весь документ.
- **Добавлено:** Восемь escape-последовательностей `\\`, `\,`,
  `\}`, `\]`, `\{`, `\[`, `\n`, `\r` внутри значений inline-скаляров
  (§ 3.7). Любая другая форма `\X` — ошибка `BadEscapeSequence`.
- **Добавлено:** Грамматика числовых литералов, охватывающая `0x`
  (шестнадцатеричные), `0o` (восьмеричные), `0b` (двоичные) и
  десятичные, с разделителями-подчёркиваниями между цифрами (§ 3.6).
  Integer Value несёт целочисленное значение; Float Value несёт
  числовое значение; оба имеют канонические текстовые формы
  (§ 5.9.8). В минимальном i64-домене переполнение big-integer
  проваливается в String; более широкий целочисленный домен MAY
  сохранить такой литерал как Integer с учётом §§ 5.2 и 8.1.
...
>>>>> lang=zh
- **Breaking:** 移除类型标记 `:i`/`:f`。数字、布尔、`null` 从标量
  字面形式推断(§ 3.6、§ 5.2)。原始标记 `::` 保留用于强制 String。
...
- **Breaking:** Float Values 不再保留文本形式;应用数值规范化
  ...
  确定性的文本形式。
- **Breaking:** 键段修剪前后 ASCII 空白(§ 4)。修剪后为空的段是
  `EmptyKey`。
- **新增:** Inline 复合值 —— `{key: value, key2: value}` 与
  `[v1, v2, v3]`,可带尾部逗号(§ 5.8)。Inline 形式可作为值、数组项
  或整个文档使用。
- **新增:** 八个 escape 序列 `\\`、`\,`、`\}`、`\]`、`\{`、`\[`、
  `\n`、`\r`,用于 inline 标量值内(§ 3.7)。任何其他 `\X` 形式都是
  `BadEscapeSequence` 错误。
- **新增:** 数字字面量语法,涵盖 `0x` 十六进制、`0o` 八进制、`0b`
  二进制、十进制,数字之间可用下划线分隔(§ 3.6)。Integer Value
  携带整数值;Float Value 携带数值;两者都有规范文本形式(§ 5.9.8)。
  在最小 i64 域上,大整数溢出回退为 String;更宽的整数域 MAY 将该
  字面量保留为 Integer,但须遵守 §§ 5.2 与 8.1。
...
- **移除:** 错误类别 `InlineNonEmptyCompound`(§ 6.7)、
  ...
  0.5.0 文档输出标签为此名称的错误。
- **变更:** Top-level kind detection(§ 5.0.1)扩展和重写 —— 首条
  内容行上闭合的 inline 复合值是根级 inline Object / Array;单独的
  `{` / `[` 是多行根开启符(见上文 Breaking)。
- **变更:** 合规性(§ 8)拆分为 parser-conforming(§ 8.1)、
  writer-conforming(§ 8.2)与 round-trip 性质(§ 8.3);实现可以
  声明其中之一或两者。
