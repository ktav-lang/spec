...
- `package.json` — `{"type":"module"}`. Он **обязателен**:
  `build_spec.mjs` динамически импортирует файлы `meta.js` юнитов, и без
  лежащего рядом `package.json` с `"type": "module"` Node считал бы файлы
  `.js` модулями CommonJS. (В корне репозитория `package.json` нет.)
...
`body-N.js` (N >= 1). Файлов `.md` под `content/` **нет** — ноль, кроме
этого самого README.md.
...
юнита он читает `meta.bodyParts` из `meta.js` и динамически импортирует
`body-1.js` .. `body-N.js` **по порядку**, затем:
