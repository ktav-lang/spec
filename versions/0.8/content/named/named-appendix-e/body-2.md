...
>>>>> lang=ru
**§ 8.1 — обязанность реализации.** § 8.1 теперь требует строгую точку
входа для парсинга (`parse_strict`
...
- Реализация без строгой точки входа вообще, или с такой, которая не
  ...
  для сверки.
>>>>> lang=zh
**§ 8.1 —— 实现的义务。** § 8.1 现在要求严格解析入口
(`parse_strict` / `loads_strict`),用于
