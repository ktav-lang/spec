...
набор, устраняя давнее внутреннее противоречие; добавлены
escape-последовательность `\uXXXX` (§ 3.7.1) и квотированные ключи
(§ 5.3.3, разделители `"` /
...
ведущий U+FEFF удаляется из документа; `(…)` больше не сохраняет
замыкающие пробельные символы содержательных строк; ведущая
неэкранированная `"`, `'` или `` ` ``
в сегменте ключа открывает квотированный сегмент вместо обычного
содержимого ключа (поэтому пара Object `"port": 1` называет ключ
`port`, а не `"port"`);
