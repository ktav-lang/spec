...
Unicode-пробелов — даже тому, который сегодня совпадает с этим
списком в точности (`char::is_whitespace()` в Rust сегодня точно
соответствует `White_Space`): разные языковые платформы расходятся
с этим списком в обе стороны (например, `str.isspace()` в Python
дополнительно считает пробельными ASCII-байты `U+001C`–`U+001F`;
класс `\s` регулярных выражений JavaScript не распознаёт
`U+0085`), и
...
原语 —— 即使某个原语当前与该列表完全一致(Rust 的
`char::is_whitespace()` 今天就与 `White_Space` 精确一致):不同语言
运行时在两个方向上都与此列表存在分歧(例如,Python 的 `str.isspace()`
额外将 ASCII 控制字节 `U+001C`–`U+001F` 视为空白;JavaScript 的
`\s` 正则字符类未能识别 `U+0085`),
