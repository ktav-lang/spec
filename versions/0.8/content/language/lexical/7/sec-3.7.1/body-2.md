...
(§ 3.7, «Escape-последовательности НЕ обрабатываются в», см. выше).
...
(§ 3.7.1 выше), но две writer-conforming реализации, выводящие
...
注释中被处理 —— 在这些语境中,六个字节 `\`、`u` 及随后四个字符是字面内容,
与其他在这些语境中未被识别的 escape 形式完全一样(§ 3.7,「Escape 序列
不在以下场景处理」,见上文)。
...
字节时,SHOULD 优先使用命名形式(`\.` 优于同一码点的 `\uXXXX` 形式,与
