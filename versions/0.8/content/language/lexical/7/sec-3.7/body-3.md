...
`\ <space>`, `\<any-other>`) — ошибка `BadEscapeSequence` (§ 6.13).
...
分类为 String,即使解码后的体本来会是关键字或数字字面量。这同样
适用于值中的 `\.` 与 `\:`。

其他任何 `\X` 形式(包括 `\#`、`\t`、`\ <space>`、
`\<any-other>`)是 `BadEscapeSequence` 错误(§ 6.13)。
...
无 escape 处理的上下文中,`\X` 字面字节序列为两个字符(`\` 后跟 `X`)。
