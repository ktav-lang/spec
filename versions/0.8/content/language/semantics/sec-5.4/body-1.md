...
1. **Raw-маркерный элемент** — любая обрезанная строка array-item,
   ...
   (`((` ... `))`).
2. **Замкнутый inline-объектный элемент** — `{ key: value, … }` в одной строке.
3. **Замкнутый inline-массивный элемент** — `[ v, v, … ]` в одной строке.
4. **Пустой inline-объект** — `{}`.
5. **Пустой inline-массив** — `[]`.
6. **Открытое составное** — строка, обрезанное содержимое которой —
...
1. **原始标记项** —— 任意 trim 后以 `::` 开头的 array-item line
   ...
   String(`((` ... `))`)。
2. **闭合 inline 对象项** —— 单行的 `{ key: value, … }`。
3. **闭合 inline 数组项** —— 单行的 `[ v, v, … ]`。
4. **空 inline 对象项** `{}`。
5. **空 inline 数组项** `[]`。
6. **开启复合** 单独的 `{` / `[` / `(` / `((`(多行开启符);将
   新的复合 scope 压入解析器栈。
7. **空多行字符串项** `()` / `(())`。
8. **其他值** —— 任意非标记体,经 § 5.2 分发以产生相应的 Value
   (关键词、数字或 String)。
