...
- **Integer** — числовой скаляр, несущий целочисленное значение.
  ...
  форма используется writer-конформными реализациями (§ 5.9).
- **Float** — абстрактный числовой скалярный носитель. Программно
  ...
  по правилу нуля из § 5.9.8. Более широкое представление хоста
  MAY содержать ненулевые конечные значения без такого кандидата
  (например, точную рациональ `1/3`), но такое значение находится вне
  заявленного домена Ktav Float и MUST NOT допускаться как Ktav
  Float. Для минимального домена binary64 преобразование decimal
  ...
  writer-конформные реализации. Value **не** сохраняет текстовую
  форму как написано; подчёркивания, выбор `e` vs `E` и ведущий
  `+` — не часть Value.
