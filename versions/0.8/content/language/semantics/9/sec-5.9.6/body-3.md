...
- **Голый String-элемент:** `<bytes>` на своей строке на
  ...
  обычный разделитель `:` удовлетворяется за счёт `<sep-end>`; префикс
...
- **Многострочный String элемент:** строка опенера `((` на
  ...
  пробелов — обоснование: § 5.6 предписывает verbatim соединять
  строки тела побайтово, поэтому verbatim сохраняет байты
  в точности; любая индентация была бы инжектирована в значение),
  строка закрытия
...
前缀是语法上有效的键,规则 6 本身对根检测同样不作此要求(例如
...
- **多行字符串项:** verbatim 开启行 `((` 在当前缩进,体行在
  缩进 0(无前导空白 —— 理由:§ 5.6 规定 verbatim 逐字节拼接
  内容行,因此 verbatim 精确保留字节;任何缩进会被注入值),
  `))` 关闭行在当前缩进。当体段修剪后恰为 `))` 时,
