...
имеет хвостовые пробельные символы (§ 3.3) на какой-либо
содержательной строке, также не представимо: stripped-форма теперь
обрезает эти хвостовые пробельные символы при выводе, так что откат
незаметно потерял бы
...
хвостовые пробельные символы внутри multi-line String, которая
также требует сегмента, обрезающегося до `))`.

Независимо от случая с хвостовыми пробельными символами выше,
тело, вынужденное перейти в stripped-форму (через сегмент,
обрезающийся в точности
до `))`), в котором каждый непустой сегмент разделяет как минимум
одну ведущую пробельную кодовую точку на одной и той же позиции,
также не
