...
- **`key: <bytes>` (plain separator + exactly one ASCII U+0020 SPACE +
  ...
  *looks* like a number to a wider-domain reader — e.g.
  ...
  same-kind guarantee is about domain-*consistent*
...
- **`key: <bytes>` (обычный разделитель + ровно один ASCII-пробел U+0020 + тело
  ...
  домен-*согласованную* классификацию; канонический писатель String
  не должен зависеть от того, какой именно домен её пишет).
...
- **`key: <bytes>`(普通分隔符 + 恰好一个 ASCII U+0020 空格 + String 体)**
  当值是非空 String,
  ...
  一致的分类;String 的规范写入器不应取决于写出它的究竟是哪个域)。
