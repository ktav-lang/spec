...
- **Ломающее:** § 3.3 меняется с
  ...
  что как минимум два мейнстрим-языковых рантайма расходятся с этим
  списком в обе стороны. Не ломающее ни для одного выпущенного релиза 0.6.x
...
- **Изменено:** `<key-char>` (§ 4) теперь допускает сырые VT
  ...
  смысл ни одного ранее валидного документа не меняется.
- **Ломающее:** § 5.9.0 (новый) определяет
  **представимые Values** — домен, на котором действуют гарантии
  канонического писателя. Голый скалярный корень, пара Object с пустым
...
- **Изменено:** § 8.1 (вместе с определением Integer в § 5) —
  ...
  где пересечён названный граничный лист (например,
...
- **Изменено:** гарантия байт-детерминизма § 5.9 ограничивается
  writer-conforming реализациями с одинаковым заявленным доменом
  Value, доменами Integer/Float и семантикой decimal-преобразования и
  округления Float. Каждая реализация MUST детерминированно применять
  свою заявленную политику преобразования; реализации с разными
  заявлениями MAY расходиться
  там, где они получают разные Values или канонические кандидаты.
- **Ломающее:** пункт о Float в § 5 и правило 14 § 5.2 — домен
  ...
  создаёт новый случай ошибки writer.
  Минимальное binary64-преобразование использует roundTiesToEven.
...
- **破坏性:** § 3.1 —— 前导字节顺序标记的处理是确定性的:
  ...
  的第一个码点、位于任何其他字节之前);规范 writer(§ 5.9)MUST NOT
...
- **变更:** `<key-char>`(§ 4)现在允许原始 VT(`0x0B`)与 FF
  ...
  `InvalidKey` 的文档,任何此前有效的文档含义均不改变。
...
- **变更:** § 8.1(连同 § 5 的 Integer 定义)—— fixture
