...
- Exposes a strict parsing entry point, and for every fixture under
  ...
  *lossy* when its lexical form differs from the canonical form of the
  ...
  exactly `lax_value`, `expected_error` (always the string
  `"LossyScalar"`), `body`, `canonical`, and non-empty `note`, with no
  extra fields.
...
- Предоставляет строгую точку входа для парсинга, и для каждой
  ...
  `name.json`
  MUST содержать ровно `lax_value`, `expected_error` (всегда строку
  `"LossyScalar"`), `body`, `canonical` и непустой `note`, без
  лишних полей.
...
- 接受 `versions/0.7/tests/valid/` 下每个 fixture 并产生与对应
  ...
  支持宽 Float 但仅 plain i64 的实现则相反;支持一个轴并不豁免其在
...
- 提供一个严格解析入口,并且对
  ...
  字符串 `"LossyScalar"`)、`body`、`canonical` 与非空 `note`,
  不得有额外字段。
