...
- **§ 5.2 — межреализационный `MUST` «тот же Value kind» теперь
  ...
  безусловный `MUST`. Гарантия теперь действует безусловно только
  вне замкнутого набора фикстур, названных в новом манифесте
  `versions/0.7/tests/valid/boundary-fixtures.json`.
- **Новый манифест `versions/0.7/tests/valid/boundary-fixtures.json`**
  даёт машинопроверяемый контракт для расхождения числовых доменов,
  которое разрешают § 8.1 / § 8.2: расходиться со своим `.json` /
  `.canonical.ktav` oracle могут ровно те пути фикстур, что
  перечислены его ключами, и только до `wider_domain_kind` /
  `wider_domain_value` / `wider_domain_canonical`, названных в
  записи, — а не до произвольного «корректного для более широкого
  домена» Value, который runner'у пришлось бы выводить самому.
  § 8.1 и § 8.2 теперь ссылаются на этот манифест вместо описания
  исключения прозой.
...
- **Манифест граничных фикстур перенесён и упрощён:
  `versions/0.7/tests/valid/boundary-fixtures.json` →
  `versions/0.7/tests/boundary-fixtures.json`, и теперь это плоский
  список путей фикстур, а не запись с `wider_domain_kind` /
  `wider_domain_value` / `wider_domain_canonical` на каждый
  элемент.** У старой более богатой схемы было две проблемы: она
  описывала замену «голого» скаляра без способа сказать, *где* в
  полном Object-оракуле фикстуры этот скаляр находится (то, что у
  каждой текущей граничной фикстуры ровно одно поле, — случайность,
  не гарантированная схемой), и она утверждала ровно один
  «широкий» исход, тогда как открытая формулировка § 5 «MAY
  поддерживать более широкое представление» допускает профили
  Float (например, decimal с большей, но конечной точностью и более
  широким диапазоном экспоненты), округляющиеся иначе, чем и вывод
  минимального домена, и вывод с точной произвольной точностью. § 8.1
  / § 8.2 теперь прямо утверждают, что реализация с более широким
  доменом *освобождена* от побайтового совпадения с перечисленной
  фикстурой, а не привязана к одной конкретной альтернативе,
  зафиксированной этим корпусом, — её корректность там регулируется
  напрямую § 5 / § 5.9. Перенос файла из `valid/` также означает,
  что runner, перечисляющий `valid/**/*.json` как фикстуры, больше
  не примет его за фикстуру.
...
- **CHANGELOG — устранено самопротиворечие**: этот раздел ранее
  ...
  черновика.
- **`boundary-fixtures.json` теперь составлен на уровне листьев, а не
  фикстур, и дополнен двумя ранее пропущенными записями.** У плоского
  списка путей фикстур было две проблемы: он опускал две фикстуры,
  которые уже существуют и действительно зависят от домена
  (`numbers/integer/big_overflow_to_string`, чьи поля `big`/`bigger`
  выходят за пределы i64; `numbers/float/min_positive_subnormal`,
  чьё точное входное значение decimal-домен с произвольной точностью
  сохраняет полностью, тогда как binary64 сокращает его до `5e-324`), —
  из-за чего соответствующая реализация с BigInt или точным decimal не
  могла пройти по ним § 8.1 / § 8.2; и он освобождал фикстуру целиком
  даже когда от домена зависели лишь некоторые её поля (поле `tiny`
  в `big_overflow_to_string` — обычный `Integer(1)` в любом домене,
  а освобождение на уровне фикстуры прекратило бы проверку и его).
  Каждая запись теперь называет JSON-указатель `path` на конкретный
  лист плюс `boundary_class` (`integer_range` / `float_range` /
  `float_underflow` / `float_precision`), так что освобождение
  реализации ограничено тем точным листом и той осью, для которых она
  действительно поддерживает более широкий домен, — реализация с более
  широким доменом Integer, но с обычным binary64 Float освобождена
  только на листьях `integer_range`, и наоборот. § 8.1 / § 8.2
  переписаны соответственно.
